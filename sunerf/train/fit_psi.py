...
validation_rho_true = psi_data['rho'][:, :, 0] * ne_scaling
...
temperature_response_config = {
    ...
    'learnable': False,
    'instruments': ['PSI']}
sampling_config = {'type': 'spherical', 'max_distance': 2.5}

absorption_model = init_absorption_model({'type': 'constant'})

# dummy module for rendering
rendering_module = PlasmaRadiativeTransfer(temperature_response_config=temperature_response_config,
                                           absorption_model=absorption_model)
rendering = BasicRenderingModule(model=model,
                                 rendering_modules={'PSI': rendering_module},
...
for epoch in range(epochs):
    ...
    for batch in tqdm(loader, total=len(loader)):
        ...
        sigma_loss = sigma.pow(2).mean()
        loss = rho_loss + T_loss + sigma_loss * 100
        assert not torch.isnan(loss).any(), 'NaN detected in loss. Stopping training.'
        loss.backward()
        optimizer.step()
        total_loss.append(loss.cpu().detach().numpy())
        total_sigma.append(sigma_loss.detach().cpu().numpy())
    print(f'Epoch {epoch}, loss: {np.mean(total_loss):.3f}, sigma: {np.mean(total_sigma):.3f}, ')
    ...
    T_norm = LogNorm(vmin=1e4, vmax=1e7)
    rho_norm = LogNorm()
    fig, axs = plt.subplots(2, 2, figsize=(10, 5))

    ax = axs[0, 0]
    im = ax.imshow(validation_rho_true, origin='lower', cmap='viridis', norm=rho_norm)
    ax.set_title('rho_true')
    fig.colorbar(im, ax=ax)

    ax = axs[0, 1]
    im = ax.imshow(validation_T_true, origin='lower', cmap='inferno', norm=T_norm)
    ax.set_title('T_true')
    fig.colorbar(im, ax=ax)

    ax = axs[1, 0]
    im = ax.imshow(10 ** log_ne_pred, origin='lower', cmap='viridis', norm=rho_norm)
    ax.set_title('rho_pred')
    fig.colorbar(im, ax=ax)

    ax = axs[1, 1]
    im = ax.imshow(10 ** mean_log_T_pred, origin='lower', cmap='inferno', norm=T_norm)
    ax.set_title('T_pred')
    fig.colorbar(im, ax=ax)
