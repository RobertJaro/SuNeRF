...
# coordinate setup
carrington_frame = frames.HeliographicCarrington(observer='mars', obstime=start_time)
planet_coord = get_body_heliographic_stonyhurst('mars', time=start_time).transform_to(carrington_frame)

start_lat = planet_coord.lat
start_lon = planet_coord.lon

target_lat = start_lat - 45 * u.deg
target_lon = start_lon - 90 * u.deg
...
points_1 = zip(np.linspace(start_lat, target_lat, n_points),
               np.linspace(start_lon, target_lon, n_points),
...
for i, (lat, lon, time, d) in tqdm(list(enumerate(points)), total=len(points)):
    outputs = loader.load_observer_image(lat, lon, time, batch_size=batch_size,
    ...
    plt.tight_layout()
    fig.savefig(os.path.join(img_path, f'{time.isoformat("T", timespec="minutes")}.jpg'), dpi=300)
    plt.close(fig)

    # write fits files
    for c, s_map in outputs['maps'].items():
        s_map.save(os.path.join(out_path, f'{time.isoformat("T", timespec="minutes")}_{c:03d}.fits'), overwrite=True)
