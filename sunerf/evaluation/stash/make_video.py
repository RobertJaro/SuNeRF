...
class VideoRender:
    ...
    def __init__(self, main_args, video_args):
        ...
        self.n_samples_hierarchical = main_config['Hierarchical sampling'][
            'n_samples_hierarchical']  # Number of samples per ray
        ...
        self.d_filter_fine = main_config['Model'][
            'd_filter_fine']  # 128     # Dimensions of linear layer filters of fine network
        self.n_layers_fine = main_config['Model'][
    ...
    def read_images_define_observer(self,
                                    distance  # TODO: This file needs to be better adapted once there is a model checkpoint
                                    =1):
        ...
        if not self.synth_data:
            ...
            for wavelength in tqdm(self.wavelengths, desc='wavelength'):
                ...
                with multiprocessing.Pool(os.cpu_count()) as p:
                    sdo_maps = p.starmap(loadAIAMap,
                                         zip(sdo_paths, repeat(self.resolution)))  # TODO Starmap works here.
                ...
                views = [
                    ((s_map.carrington_latitude.to(u.deg).value, -s_map.carrington_longitude.to(u.deg).value), s_map)
                    for s_map in sdo_maps]
        ...
        self.near, self.far = (1 * u.AU - 1.2 * u.solRad).to(u.solRad).value, (1 * u.AU + 1.2 * u.solRad).to(
            u.solRad).value
    ...
    def save_frame_as_jpg(self, i, point, wavelength, half_fov=1.3):
        ...
        outputs = self.models[wavelength].load_observer_image(lat, -lon, time, focal=focal, batch_size=self.batch_size,
                                                              strides=self.strides,
                                                              r_decay=self.r_decay, tau_decay=self.tau_decay,
                                                              r_cut=self.r_cut)
    ...
    def save_frame_as_fits(self, i, point, wavelength, half_fov=1.3):
        ...
        # Create new header
        new_observer = SkyCoord(-lon * u.deg, lat * u.deg, r * u.AU, obstime=self.ref_map[wavelength].meta['t_obs'],
    ...
    def plot_euv_image(self, s_map, ax, wavelength, cmap, vmin=0, vmax=1):
        ...
        ax.text(0.01, 0.99, wavelength, horizontalalignment='left', verticalalignment='top', color='w',
    ...
    def draw_irradiance(self, axs, irradiance, sun_distance=1):
        ...
        axs.bar(np.arange(0, len(irradiance)), irradiance / sun_distance ** 2, width=0.5, alpha=0.5, ecolor='red',
                capsize=8, color='#1c79f2')
    ...
    def draw_orbit(self, ax, point_sp, point_car):
        ...
        legend_dt.append(Line2D([0], [0], color='None', lw=0, marker='o', mec='yellow', mfc='yellow', label='Sun',
                                ms=300 / legend_size_factor))

        for i in np.arange(1, 7):
            ax.plot(self.planets[i]['pos'][:, 0], self.planets[i]['pos'][:, 1], self.planets[i]['pos'][:, 2],
                    c='#75756d', zorder=0, lw=lw)
            s = self.sizes[i - 1] / (1 + np.sqrt(np.sum((self.planets[i]['pos'][0, :] - ref_pos) ** 2)))
            ax.scatter(self.planets[i]['pos'][0, 0], self.planets[i]['pos'][0, 1], self.planets[i]['pos'][0, 2],
                       c=self.colors[i - 1], s=s, zorder=zorder[i])
            if i < 5:
                legend_dt.append(
                    Line2D([0], [0], color='None', lw=0, marker='o', mec=self.colors[i - 1], mfc=self.colors[i - 1],
                           label=self.names[i - 1], ms=self.sizes[i - 1] / legend_size_factor))
        ...
        ax.text2D(0.01, 0.99, f'r: {np.round(r, 2)} AU\n$\lambda$: {np.round(lat, 1)}°\n$\phi$: {np.round(lon, 1)}°',
                  horizontalalignment='left', verticalalignment='top', color='w', transform=ax.transAxes)
    ...
    def camera_path(self, posOne, posTwo, n=180, r=1):
        ...
        return np.stack((Pone2Ptwo_r, Pone2Ptwo_lat, Pone2Ptwo_lon), axis=1), np.stack(
            (Pone2Ptwo_x, Pone2Ptwo_y, Pone2Ptwo_z), axis=1)

    def run_irradiance_inference(self):
        r"""method to run irradiance inference on loaded stacks
        """

        view_files = [sorted(glob.glob(f'{self.path_to_save_video}/{wavelength}/*.fits')) for wavelength in
                      self.wavelengths]
        view_files = np.array(view_files).transpose()
        view_ds = FITSDataset(view_files, aia_preprocessing=False)

        total_irradiance = [irr for irr in
                            ipredict(self.irradiance_model_path, view_ds, self.eve_normalization, return_images=False)]
