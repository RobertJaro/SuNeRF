...
from sunerf.evaluation.loader import SuNeRFLoader, ConditionedSuNeRFLoader
...
for i, (lat, lon, d) in tqdm(list(enumerate(points)), total=len(points)):
    outputs = loader.load_image(lat * u.deg, lon * u.deg, distance=d * u.AU, batch_size=batch_size,
