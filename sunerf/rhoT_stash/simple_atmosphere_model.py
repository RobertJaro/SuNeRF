...
def simple_density_temperature(x, y, z, t, h0=60 * u.Mm, t0=1.2e6 * u.K, R_s=1.2 * u.solRad, t_photosphere=5777 * u.K,
    ...
    rho[else_index] = rho_0 * np.exp(
        1 * u.solRad / h0 * (1 * u.solRad / radius[else_index] - 1))  # See equation 4 in Pascoe et al. 2019
    ...
    temp[R_s_index] = (radius[R_s_index] - 1 * u.solRad) * (
                (t0 - t_photosphere) / (R_s - 1 * u.solRad)) + t_photosphere  # See equation 6 in Pascoe et al. 2019
...
def _retrieve_query_points(imgs_path, resolution=1024):
    ...
    views = [((s_map.carrington_latitude.to(u.deg).value, -s_map.carrington_longitude.to(u.deg).value), s_map) for s_map
             in [*sdo_maps]]
    ...
    all_rays = torch.stack([torch.stack(get_rays(height, width, focal, p), -2)
                            for p in torch.tensor(poses[0:1])],
                           0)  # shape is [number of poses, height, width, ray_o and ray_d, (x,y,z)]
