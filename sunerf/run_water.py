...
from sunerf.data.loader.water_vapor import WaterVaporDataModule
from sunerf.model.water import NEarthFModule
from sunerf.train.callback import WaterImageCallback, WaterSliceCallback
...
if __name__ == '__main__':
    ...
    if os.path.exists(data_module_save_path) and not args.reload:
        ...
        data_module = WaterVaporDataModule(**data_config, work_directory=work_directory)
        torch.save(data_module, data_module_save_path)

    # initialize SuNeRF model
    sunerf = NEarthFModule(instruments=instruments,
                           meters_per_ds=data_module.meters_per_ds, seconds_per_dt=data_module.seconds_per_dt,
    ...
    for callback_config in config.get('callbacks', []):
        ...
        if callback_type.lower() == 'image':
            callback = WaterImageCallback(ds_key=ds_key,
                                          image_shape=data_module.validation_datasets[ds_key].image_shape)
        elif callback_type.lower() == 'slice':
            callback = WaterSliceCallback(ds_key=ds_key, cube_shape=data_module.validation_datasets[ds_key].cube_shape,
                                          meters_per_ds=data_module.meters_per_ds)
        else:
